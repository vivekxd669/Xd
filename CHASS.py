try:
    ...
    os.system('title CHASS - Installing dependencies')
    i = 0
    imports = [
        'requests', 
        'colorama', 
...
class Render:
    ...
    def render_ascii(self):
        os.system('cls')
        os.system(f"title CHASS - .gg/eternalsop")
        edges = ["╗", "║", "╚", "╝", "═", "╔"]
        title = f"""
...
class Solver:
    ...
    def solve_capsolver(website_url, website_key):
        ...
        response = session.post('https://api.capsolver.com/createTask', headers=headers, json=json)
        try:
        ...
        while True:
            time.sleep(1.5)
            response = session.post('https://api.capsolver.com/getTaskResult', headers=headers, json=json)
                ...
                case 400:
                    ...
                    if solver == "True":
                        if service == "capsolver":
                            payload = {
                                "captcha_key": Solver().solve_capsolver(site_key='4c672d35-0701-42b2-88c3-78380b0db560', page_url=f'https://discord.com/')
                            }
                        else:
                            payload = {
                                "captcha_key": Solver().solve_2cap(site_key='4c672d35-0701-42b2-88c3-78380b0db560', page_url=f'https://discord.com/')
                            }
...
class Menu:
    ...
    @wrapper
    def soundbord(self):
        os.system('title CHASSS - Soundboard Spam')
    ...
    @wrapper
    def friender(self):
        os.system('title CHASS - Friender')
    ...
    def nick_changer(self):
        os.system('title CHASS - Nickname Changer')
    ...
    def voice_joiner(self):
        os.system('title CHASS - Voice Joiner')
        Link = input(console.prompt("Channel LINK"))
        if Link == "":
            Menu().main_menu()
        if Link.startswith("https://"):
    ...
    def Thread_Spammer(self):
        os.system('title CHASS - Thread Spammer')
    ...
    def checker(self):
        os.system('title CHASS - Checker')
        self.raider.token_checker()

    @wrapper
    def reactor(self):
        os.system('title CHASS - Reactor')
    ...
    def formatter(self):
        os.system('title CHASS - Formatter')
        self.run(self.raider.format_tokens, [()])
    
    @wrapper
    def button(self):
        os.system('title CHASS - Clicker')
    ...
    @wrapper
    def accept(self):
        os.system('title CHASS Accept Rules')
    ...
    @wrapper
    def onboard(self):
        os.system('title CHASS - Onboarding Bypass')
        guild_id = input(console.prompt("Guild ID"))
        if guild_id == "":
            Menu().main_menu()
        os.system('cls')
